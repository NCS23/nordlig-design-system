...
export const DesignTokens: Story = {
  ...
  render: () => {
    const tokens = [
      {
        token: '--color-datatable-toolbar-bg',
        value: '#ffffff',
        usage: 'Toolbar- und Header-Hover-Hintergrund',
      },
      {
        token: '--color-datatable-row-selected-bg',
        value: '#f1f5f9',
        usage: 'Hintergrund ausgewaehlter Zeilen',
      },
      {
        token: '--color-datatable-sort-icon',
        value: '#475569',
        usage: 'Inaktives Sort-Icon',
      },
      {
        token: '--color-datatable-sort-icon-active',
        value: '#0284c7',
        usage: 'Aktives Sort-Icon (auf-/absteigend)',
      },
      {
        token: '--color-datatable-empty-text',
        value: '#475569',
        usage: 'Leerzustand-Text und Seiteninformation',
      },
      {
        token: '--spacing-datatable-toolbar-padding',
        value: '16px',
        usage: 'Padding der Toolbar und Pagination',
      },
      {
        token: '--spacing-datatable-toolbar-gap',
        value: '12px',
        usage: 'Abstand zwischen Toolbar, Tabelle und Pagination',
      },
    ];
    ...
    return (
      <div style={{ padding: '24px' }}>
        ...
        <p style={{ fontSize: '14px', color: '#475569', marginBottom: '24px' }}>
          Zusaetzlich werden alle Table-Tokens aus table.json verwendet.
        </p>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #e2e8f0' }}>
              <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 600 }}>Token</th>
              <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 600 }}>Wert</th>
              <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 600 }}>Verwendung</th>
            </tr>
          </thead>
          <tbody>
            {tokens.map((t) => (
              <tr key={t.token} style={{ borderBottom: '1px solid #e2e8f0' }}>
                ...
                <td style={{ padding: '8px 12px', color: '#475569' }}>{t.usage}</td>
              </tr>
            ))}
          </tbody>
        </table>
