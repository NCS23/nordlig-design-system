...
const renderDropdown = (props?: { onSelect?: () => void }) => {
  return render(
    <DropdownMenu>
      <DropdownMenuTrigger>
      ...
      <DropdownMenuContent>
        ...
        <DropdownMenuItem onSelect={props?.onSelect}>Bearbeiten</DropdownMenuItem>
        <DropdownMenuItem icon={<svg data-testid="icon" />}>Mit Icon</DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem disabled>Deaktiviert</DropdownMenuItem>
...
describe('DropdownMenu', () => {
  ...
  it('applies destructive styling', async () => {
    ...
    renderDropdown();
    await user.click(screen.getByRole('button', { name: 'Aktionen' }));
    const deleteItem = screen.getByText('Löschen').closest('[role="menuitem"]')!;
